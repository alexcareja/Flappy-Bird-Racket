...
(define-struct state (b pipes score var) #:transparent)
(define (get-bird s) (state-b s));
(define (get-pipes s) (state-pipes s))
...
(define (get-pipe-x p) (pipe-x p))
(define (get-pipe-gap-y p) (pipe-gap-y p))

  
(define-struct var (momentum gravity scroll-speed) #:transparent)
(define (get-variables-momentum v) (var-momentum v))
...
(define (get-initial-state)
  (state init-bird (list init-pipe) 0 init-var))
